...
def sparse_coding(images, sc1, sc2, sc3, train_sc1=False, train_sc2=False, train_sc3=False):
    ...
    image_transformed1, _ = sc1.forward(sample_blocks, train=train_sc1)# 第1次稀疏编码
    ...
    sample_blocks2 = sliding_window_sampling(image_reconstructed1)# 第2次稀疏编码
    ...
    image_transformed3, _ = sc3.forward(image_reconstructed2, train=train_sc3)# 第3次稀疏编码
...
def main(train_epochs, encode_test):
    ...
    print(" 1")
    for epoch in range(train_epochs):#xunliandiyicengzidian
    ...
    print(" 2")
    for epoch in range(train_epochs):#训练第二层字典，固定第一层
        for batch_idx, (images, labels) in enumerate(train_loader):
            sample_blocks = sliding_window_sampling(images)
    ...
    print(" 3")
    for epoch in range(train_epochs):#训练第三层字典，固定第一，二层
        for batch_idx, (images, labels) in enumerate(train_loader):
            sample_blocks = sliding_window_sampling(images)
            image_transformed1, _ = sc1.forward(sample_blocks, train=False)
            image_reconstructed1 = image_transformed1.reshape(images.size(0), 20, 13, 13)
